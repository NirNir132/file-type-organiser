...
const App: React.FC = () => {
  ...
  const [isLoadingInitialScan, setIsLoadingInitialScan] = useState<boolean>(false);
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    setError(null);
    try {
      const zipBlob = await createZipFromFiles(filteredFiles, `organized_${targetExtension}.zip`);
      ...
      setError(err instanceof Error ? err.message : "Failed to create ZIP file. Check console for details.");
    } finally {
      setIsZipping(false);
    }
  ...
  useEffect(() => {
    ...
    if (!targetExtension.trim() && showFilteredList && !error?.includes("file extension to filter by")) {
       setFilteredFiles([]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-12 sm:py-20 px-4 sm:px-6 lg:px-8 selection:bg-sky-200 selection:text-sky-900">
      <header className="mb-12 md:mb-16 text-center">
        ...
        <p className="mt-4 text-lg text-slate-700/90 max-w-2xl mx-auto">
          Effortlessly drag & drop a folder, specify a file type, and instantly download your neatly organized files.
        </p>
      </header>
      ...
      <main className="w-full max-w-2xl lg:max-w-3xl space-y-10">
        {error && (
          ...
          >
            ...
            <AlertTriangle className="w-6 h-6 text-red-600 flex-shrink-0 mt-0.5" strokeWidth={2.5} />
            <div>
              <span className="font-semibold block text-base">Oops! An error occurred:</span>
        ...
        {isLoadingInitialScan && (
          <div className="flex flex-col items-center justify-center p-10 bg-white/80 backdrop-blur-md shadow-2xl rounded-2xl">
            ...
            <p className="text-slate-700 text-lg font-semibold">Scanning Folder Contents...</p>
            <p className="text-slate-500/90 text-sm">Please wait, this may take a moment for large folders.</p>
          </div>
        )}

        {!isLoadingInitialScan && (allScannedFiles.length > 0 || targetExtension || showFilteredList) && (
          <FileFilter
      ...
      <footer className="mt-20 pt-10 border-t border-slate-300/70 w-full max-w-3xl text-center text-sm text-slate-600/90">
        <p>&copy; {new Date().getFullYear()} File Organizer App. All rights reserved.</p>
        <p className="text-xs text-slate-500/80 mt-1">Crafted with React, Tailwind CSS, and a sprinkle of innovation.</p>
      </footer>
    </div>
  );
};
