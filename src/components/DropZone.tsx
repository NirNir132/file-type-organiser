...
interface DropZoneProps {
	onFilesScanned: (files: AppFile[]) => void;
		...
		async (event: React.DragEvent<HTMLDivElement>) => {
			...
			if (event.dataTransfer.items && event.dataTransfer.items.length > 0) {
				...
				try {
					...
					if (
						files.length === 0 &&
						event.dataTransfer.items[0]?.webkitGetAsEntry()?.isFile
					) {
						setAppError(
							"Only folders can be dropped. Please drop a folder, not individual files."
						);
						onFilesScanned([]);
					} else if (files.length === 0) {
						setAppError(
							"No files found in the dropped folder or could not access folder contents. Please try a different folder."
						);
						onFilesScanned([]);
					} else {
						onFilesScanned(files as AppFile[]);
					}
				} catch (error) {
					console.error("Error processing dropped items:", error);
					setAppError(
						"Failed to process dropped folder. Ensure it's a valid folder and try again. Check console for details."
					);
					onFilesScanned([]);
				} finally {
				...
				setAppError(
					"Your browser doesn't support folder dropping in this way, or the dropped item was not a folder. Try a different browser or method."
				);
				onFilesScanned([]);
				setAppIsLoading(false);
			}
      ...
      ${
		...
		>
			<div className="flex flex-col items-center justify-center space-y-4 pointer-events-none">
				...
				/>
				<p
					className={`text-lg sm:text-xl font-semibold transition-colors duration-300 ease-in-out ${
						isDragging
							? "text-sky-700"
							: "text-slate-700/90 group-hover:text-sky-600"
					}`}
				>
					{isDragging ? "Release to Scan Folder!" : "Drag & Drop Folder Here"}
				</p>
				<p
					className={`text-sm sm:text-base transition-colors duration-300 ease-in-out ${
						isDragging
							? "text-sky-600/90"
							: "text-slate-500/80 group-hover:text-sky-500/90"
					}`}
				>
